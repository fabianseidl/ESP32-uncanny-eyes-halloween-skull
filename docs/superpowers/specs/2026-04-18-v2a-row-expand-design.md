...
**Measured FPS (v2a left-eye, default_large.h, 240→466 NN, commit `cd2020d`):** 10 fps sustained — at the gate, short of the 20-fps stretch. Wall clock is QSPI/driver-bound: 466² × 2 B × 10 fps ≈ 35 Mbps effective against a ~160 Mbps peak QSPI bus, with the remainder spent on synchronous `display_writePixels()` overhead and per-call CO5300 command framing. Hoisting per-row base pointers and the `irisY` range check out of `drawEyeRow()`'s inner loop (commit `cd2020d`) produced no wall-clock FPS change, confirming the renderer is not CPU-bound at the current ceiling. Cracking ≥ 20 fps requires an async-DMA wrapper on `Arduino_ESP32QSPI` or a smaller-per-frame strategy — see Future Work "v2a-perf: renderer optimization" and README "ideas for later".
